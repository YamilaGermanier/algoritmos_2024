...
###################### parcial
    def mostrar_por_tipo(self, tipo):
        def __mostrar_por_tipo(root, tipo):
            if root is not None:
                ...
                if tipo in root.other_value['tipo']:
    ...
    def contar_por_tipo(self, tipo):
        def __contar_por_tipo(root, tipo):
            count = 0
            if root is not None:
                if tipo in root.other_value['tipo']:
                    count += 1
                count += __contar_por_tipo(root.left, tipo)
                count += __contar_por_tipo(root.right, tipo)
            return count
        
        return __contar_por_tipo(self.root, tipo)

    def listar_por_numero(self):
        def __listar_por_numero(root):
            if root is not None:
                __listar_por_numero(root.left)
                print(root.other_value['nombre'], root.other_value['numero'])
                __listar_por_numero(root.right)

        __listar_por_numero(self.root)
